...
namespace CG
{
    ...
    Image::Image(const char* FilePath) : Pixels(nullptr), width(0), height(0), BitsPerPixel(0)
    {
        ...
        this->Pixels = new BGRA[width * height];
        this->ProcessPixels(Data.data(), this->Pixels);
    }

    Image::Image(int Width, int Height) : Pixels(new BGRA[Width * Height]), width(Width), height(Height), BitsPerPixel(32)
    {
        std::memset(this->Pixels, 0, Width * Height);
    }

    Image::Image(BGRA* Ptr, int Width, int Height) : Pixels(Ptr), width(Width), height(Height), BitsPerPixel(32)
    {
        PointerInit = true;
    }

    #if defined _WIN32 || defined _WIN64
    Image::Image(CURSORINFO &Info) : Pixels(nullptr), width(0), height(0), BitsPerPixel(32)
    {
        ...
        if (this->width && this->height)
        {
            ...
            BITMAPINFO BMInfo = {sizeof(BITMAPINFOHEADER), static_cast<long>(width), static_cast<long>(height), 1, BitsPerPixel, BI_RGB, static_cast<unsigned int>(Data.size()), 0, 0, 0, 0};
            GetDIBits(SDC, II.hbmColor, 0, height, Data.data(), &BMInfo, DIB_RGB_COLORS);
            this->Pixels = new BGRA[Data.size() / sizeof(BGRA)];
            this->ProcessPixels(Data.data(), this->Pixels);
        }
    ...
    Image::Image(HDC DC, int X, int Y, int Width, int Height) : Pixels(nullptr), width(Width), height(Height), BitsPerPixel(32)
    {
        ...
        HGDIOBJ Old = SelectObject(SDC, hSBmp);

        BitBlt(SDC, 0, 0, width, height, DC, X, Y, SRCCOPY);
        std::vector<std::uint8_t> Data(((width * BitsPerPixel + 31) / 32) * 4 * height);

        BITMAPINFO Info = {sizeof(BITMAPINFOHEADER), static_cast<long>(width), static_cast<long>(height), 1, BitsPerPixel, BI_RGB, static_cast<unsigned int>(Data.size()), 0, 0, 0, 0};
        GetDIBits(SDC, hSBmp, 0, height, Data.data(), &Info, DIB_RGB_COLORS);

        DeleteObject(SelectObject(SDC, Old));
        DeleteDC(SDC);
        DeleteObject(hSBmp);
        ReleaseDC(nullptr, MemDC);
        this->Pixels = new BGRA[Data.size() / sizeof(BGRA)];
    ...
    Image::Image(HWND Window, int X, int Y, int Width, int Height) : Pixels(nullptr), width(Width), height(Height), BitsPerPixel(32)
    {
        HDC DC = GetDC(Window);
        BITMAP Bmp = {0};
        HBITMAP hBmp = reinterpret_cast<HBITMAP>(GetCurrentObject(DC, OBJ_BITMAP));
    ...
    void Image::ProcessPixels(const std::uint8_t* In, BGRA* Out)
    {
    ...
    void Image::ProcessPixels(const BGRA* In, std::uint8_t* Out)
    {
        for (std::size_t I = 0; I < height; ++I)
        {
            for (std::size_t J = 0; J < width; ++J)
